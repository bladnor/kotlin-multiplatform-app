...
val url = "http://localhost:8080/events"
...
fun addEvent(event: Event) {
    ...
    window
            .fetch(url, RequestInit(method = "POST", headers = headers, body = body, referrerPolicy = "origin", cache = RequestCache.NO_CACHE, redirect = RequestRedirect.MANUAL))
            .then {
                when (it.status) {
                    200.toShort() -> {
